...
public class Selection {
    public static void main(String[] args) {
        var exemplo = new int[] { 4, 9, 2, 1, 7, 8, 0, 3, 15, 11 };
    ...
    public static int[] selection(int[] desordenada) {
        ...
        for (int i = 0; i < desordenada.length - 1; i++) {
            indiceMenor = i;
            for (int ii = i; ii < desordenada.length; ii++) { // ao final do for anterior, diminui o tamanho que percorre
                if (desordenada[ii] < desordenada[indiceMenor]) {
                    indiceMenor = ii;
                }

                if (desordenada[i] > desordenada[ii]) {
                    copia = desordenada[ii];
                    desordenada[ii] = desordenada[i];
                    desordenada[i] = copia;
                }
            }
        }
